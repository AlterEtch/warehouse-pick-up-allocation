...
class Task():
    def __init__(self, canvas, world, pos, index=0, cost=1, isStation=False, mean=0.05, timeout=300):
        ...
        if not self.isStation:
            self.id = self.canvas.create_oval(self.pos[0] * world.gridSize + 0.5 * (world.gridSize - self.size),
                                              self.pos[1] * world.gridSize + 0.5 * (world.gridSize - self.size),
                                              (self.pos[0] + 1) * world.gridSize - 0.5 * (world.gridSize - self.size),
                                              (self.pos[1] + 1) * world.gridSize - 0.5 * (world.gridSize - self.size),
                                              fill="gray30")
            self.id_label = self.canvas.create_text((self.pos[0] + 0.5) * world.gridSize, (self.pos[1] + 0.5) * world.gridSize,
