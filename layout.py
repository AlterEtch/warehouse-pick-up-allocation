...
def get_layout3():
    width, height, gridSize = 820, 620, 20
    wallLayout = [[0 for row in range(0, height / gridSize + 1)] for col in range(0, width / gridSize + 1)]
    for x in range(0, width / gridSize):
        for y in range(0, height / gridSize):
            if x in {0, 0, width / gridSize, width / gridSize - 1} or y in {0, 0, height / gridSize - 1,
                                                                            height / gridSize - 1}:
                wallLayout[x][y] = 1
...
def get_layout4():
    width, height, grid_size = 820, 580, 20
    wall_layout = [[0 for row in range(0, height / grid_size)] for col in range(0, width / grid_size)]
    for x in range(0, width / grid_size):
        for y in range(0, height / grid_size):
    ...
    for m in range(1, width / grid_size - 2):
        for n in range(1, height / grid_size - 1, 8):
            wall_layout[m][n] = 0
            wall_layout[m + 1][n] = 0
            wall_layout[m][n + 1] = 0
            wall_layout[m][n + 2] = 0
